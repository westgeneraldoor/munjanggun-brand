...
나머지 397개로 확대하기 전에 정지 이미지 12개로 전체 절차를 한 번 완주했다.

- 1차 원문 12건과 독립 2차 원문 12건을 수정하지 않은 바이트 그대로 보존했다.
- 원문 6개 배치는 실제 작성자 3명의 서로 다른 Ed25519 키로 exact-byte attestation을 만들었다.
- 두 원문의 의미 차이는 원본 이미지를 다시 열어 자산별 별도 교정본 12개에 기록했다.
- `눈을 넓히고`, 우측 상단 `문장군`, Q/A 라벨, 문구 없는 이미지의 무문자 상태를 원본과 대조했다.
- 1·2차 작성자와 다른 교정자 키로 12개 교정본을 서명했다.
- 검증기는 원본·원문·pair·서명·신뢰키·교정본의 경로와 SHA, 작성자 독립성, 시간 순서, 12개 수량을 다시 확인한다.
- v2 교정본은 정규화된 1차 원문을 기준으로 모든 1·2차 차이와 최종값 변경을 명시적인 원자 필드 결정으로 남긴다. 검증기는 이 결정들을 적용해 최종 관찰 전체를 재구성하고 제출본과 완전 비교하므로, 결정 밖 문구·가격/A/S 신호·배열 변경은 통과할 수 없다.
...
- 12~23번 세그먼트는 1차 12건·2차 12건·pair 12건과 raw batch 작성자 서명 5개를 먼저 `attested_non_authority`로 검증했다.
- 검증기는 실제 확인한 서명 수와 원장의 `attestedBatchCount`도 일치시켜 숫자만 잘못 적은 원장을 거절한다.
- 독립 원본 대조 뒤 v2 교정본 12개와 원자 결정 69개를 교정자 별도 키로 서명했다. 전체 재구성 검증 결과는 `complete_non_authority`다.
- 13번 가격카드는 가격만 관찰하고 행사 신호는 제거했으며, 17번 몰딩·도배 순서는 일정 안내로만 보존하고 사양 신호를 제거했다. 19번 작은 문구는 사장 확인에 따라 `핸드커버래핑` certain으로 정정했고, 이전 `핸드커버댐핑` 판독은 superseded 증거로만 보존했다.
- 20번은 작업자 얼굴 일부가 보이나 식별 가능성을 확정하기 어렵다는 관찰을 보존했다. 이는 관찰 충돌이 정리됐다는 뜻일 뿐 개인정보 외부 사용 허용이 아니며, 실제 발행 단계 차단은 유지한다.

```powershell
npm run assets:validate-raw-review-ledger -- `
  --ledger "Z:\문장군_브랜드_원본보관\VISUAL-REVIEW-2026-09-08\raw-review-ledger-v1\LEDGER_INDEX.json" `
  --mode pilot-complete `
  --reviewer-trust "Z:\문장군_브랜드_원본보관\VISUAL-REVIEW-2026-09-08\raw-review-ledger-v1\reviewer-trust.json"
```
