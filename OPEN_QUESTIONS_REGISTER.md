...
| OQ-022 | 정식 로고 · 파비콘 제작 확정 | `DESIGN.md` v4.0과 `BRAND_GUIDELINE.html`은 라운드 프레임 SVG를 "정식 로고 제작 전 단계의 방향"으로만 제시함. 실제 로고 파일(벡터, 파비콘, 앱 아이콘)이 없어 홈페이지/블로그/상담접수 웹앱이 각자 다른 마크를 쓸 위험 | 컨셉 마크만 존재: 라운드 사각 프레임 + 세이지 중심 분할선 + 오커 손잡이 2점. 최소 크기 96px(인라인)/24px(파비콘) 가이드만 있고 실제 파일 없음 | 정식 로고 제작 여부, 제작 주체(내부/외주), 확정 일정을 사장 확인. 확정 시 SVG/PNG/파비콘 세트를 `assets/`에 등록하고 DESIGN.md 6장 갱신 | DESIGN.md, BRAND_GUIDELINE.html | `open` |
