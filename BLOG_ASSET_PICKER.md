...
2026-09-07 `verified-v4`는 내용 오류 때문에 철회됐다. 이후 두 intake를 합쳐 정지 이미지 409개와 GIF 80개의 독립 판독·비교·교정을 비공개 원장으로 완료했지만, 결과는 아직 `non_authority`다. content-evidence용 픽셀·문구·2차 verdict 증거와 승격 기록이 완성되기 전까지 `assets:library`, `assets:library:index`, `assets:search`, `assets:pick-for-blog`의 운영 검색과 handoff는 `blocked_pending_visual_revalidation`로 중지돼 있다.
...
따라서 이전의 “직접 픽셀 확인 322개가 남았다”는 단계와 자산별 1차 구조화는 끝났다. 최종 1차 후보는 489/489 준비·미해결 관찰 0이고, 정지 관찰 2,372건과 GIF 관찰 410건에 연결된 고유 픽셀 증거 2,279개를 원본에서 역검증했다. 이 후보의 exact bytes도 전용 1차 검토자 키로 서명했다. 현재 남은 단계는 **다른 검토자와 다른 키**가 최종값에 결속해 수행하는 489개 독립 2차 의미 확인, 민감 claim 문구 2차 확인, claim 389개·privacy 50개 발행 판정, GIF 80개 연속 원본 재생 또는 전 프레임 검토, intake별 content authority 봉인이다. 같은 1차 검토자가 2차 서명을 대신하면 안 되므로 승격 가능 수는 아직 0이고 운영 차단은 유지한다.
