...
const TRUSTED_PRIVATE_ROOTS = ['C:/Users/hjh/안티그래비티/문장군_브랜드_private', 'Z:/문장군_브랜드_원본보관'].map((value) => resolve(value));
...
export async function authorizePrivateRecovery({
  ...
  if (root === target) throw new Error('Recovery output must be a child of --approved-private-root, not the root itself');
  const trustedRoot = TRUSTED_PRIVATE_ROOTS.some((value) => isContained(value, root));
  if (!trustedRoot) throw new Error('Approved private root is not in the trusted private-root policy');
  if (root === target) throw new Error('Recovery output must be a child of --approved-private-root, not the root itself');
  if (!isContained(root, target)) throw new Error('Recovery output must stay under --approved-private-root');
  if (isContained(objects, target) || isContained(target, objects)) throw new Error('Recovery output and object root must be separate');
  ...
  for (const segment of segments) {
    ...
    try {
      ...
      if (info.isSymbolicLink()) throw new Error('Recovery output path must not contain symbolic links');
    } catch (error) {
      if (error.code === 'ENOENT') break;
      throw error;
    }
  }
  if (!isContained(rootReal, resolve(rootReal, ...segments))) throw new Error('Recovery output real path escapes approved root');
