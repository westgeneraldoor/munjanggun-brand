...
export async function runAssetsMaterialize(argv, { trustedPrivateRoots, emit = console.log } = {}) {
  ...
  try {
    ...
    const result = { result: 'success', assetCount: assets.length, outputRoot, receiptPath: resolve(outputRoot, '_asset-recovery-receipt.json') };
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  await runAssetsMaterialize(process.argv.slice(2));
}
