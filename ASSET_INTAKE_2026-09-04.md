...
2026-09-07 실제 원본 대조에서 설명 오분류와 가격·기능 claim 누락이 확인됐다. 기존 `reviewed`는 개수·SHA·필드 완전성과 contact sheet/OCR 확인을 뜻할 뿐, 원본별 의미 설명 정확성을 입증하지 못한다. 따라서 현재 407개 카탈로그는 `config/asset-content-quality.json`에서 `blocked_pending_visual_revalidation`이며, 원본별 full-resolution 확인과 GIF 전체 루프 확인이 끝날 때까지 검색·추천·handoff에 사용할 수 없다.
