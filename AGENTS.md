...
`BRAND_GUIDELINE.html`은 현재 디자인 기준을 브라우저에서 확인하는 시각 가이드다. `legacy-preview-v3.html`은 폐기된 v3.0 참고 이력이며, 최신 권위 기준은 항상 `DESIGN.md`다.
...
- CTA는 클레이(`--mg-clay`)를 사용하고, 보조 액션은 세이지(`--mg-sage`)를 사용한다. 오커(`--mg-ochre`)는 아이콘·소형 포인트에만 절제해서 쓴다. 네이비 계열은 v4.0(동네 온기)에서 폐지되어 사용하지 않는다.
