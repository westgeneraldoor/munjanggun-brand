...
`BRAND_GUIDELINE.html`은 현재 Editorial Showroom 디자인 기준을 브라우저에서 확인하는 시각 가이드다. 최신 권위 기준은 항상 `DESIGN.md`이며, 공통 React 컴포넌트 계약과 상태 플레이그라운드는 `design-system/`에 분리한다.

중앙 브랜드는 시각·브랜드·공통 컴포넌트 계약만 관리한다. 실제 화면, URL, 메뉴, 정보구조, 고객 여정, CTA 연결은 하위 프로젝트 권한이다. 플레이그라운드는 제품 시안이나 구현 오더가 아니다. 서비스 프로젝트는 플레이그라운드 화면을 복사하지 않고 토큰과 공통 컴포넌트 계약만 적용한다.
...
- 주요 CTA와 정보 위계는 Ink semantic 토큰을 사용하고, 상담·도움·선택·포커스는 Forest semantic 토큰을 사용한다. 화면 구현은 primitive 색상값 대신 semantic 또는 component 토큰을 소비한다.
