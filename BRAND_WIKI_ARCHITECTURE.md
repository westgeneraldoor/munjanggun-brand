...
| 스토리 문맥층 | 폴더·파일 순서를 상품/랜딩/행사의 한 이야기와 선택지 관계로 연결 | `config/product-story-contexts.json` |
...
세부 색상·코드·사양 검색에서는 직접 근거와 상위 그룹 참고자료를 구분한다. 직접 문구·코드가 확인된 자산을 먼저 제시하고, 같은 그룹에 속할 뿐 해당 세부 조건을 직접 보여주지 않는 자산은 참고자료로 표시한다. 해석하지 못한 조건을 그룹 소속만으로 충족 처리하지 않는다. 복수 상품이나 같은 선택 축의 비교 요청은 한 상품 또는 자산 교집합으로 축소하지 않고 대상을 보존해 별도 검색하도록 한다.

`resultCoverage`의 근거 역할 충족과 실제 집필 완성은 서로 다른 상태다. 상품 전체 글은 선택 축·정확한 세부 근거·조합 제한·비교 대상·현장 확인 기준에 답하는지 별도 검수하며, 현장 적합성은 `FIELD_JUDGMENT_RULES.md`를 따른다.
...
## 8-1. 개별 자산과 전체 스토리의 관계

개별 이미지 의미 검색과 상품 전체 이해는 별도 단계다. 검색 상위 이미지 몇 장이 상품의 전체 옵션을 대표한다고 가정하지 않는다.

```text
상품/주제 요청
→ product story context로 전체 목차·선택지 확인
→ 요청이 전체인지 일부인지 판정
→ 개별 자산 검색
→ 각 자산의 narrative placement 확인
→ 누락 선택지 점검
→ 집필·기획
```

`config/product-story-contexts.json`은 다음을 구조화한다.

현재 이 문맥은 중앙 상품 위키가 `vetted`인 3연동중문, 원슬라이딩, 3연동 자동, 3연동 ㄱ자, 스윙, 양개형/미서기, ABS도어 방문교체, ABS도어 문틀리폼 필름시공 8개 상품군에 연결한다. `candidate` 단계의 신규 상품과 쇼핑스토리 주제는 중앙 사실·주제 위키가 생기기 전까지 자동 승격하지 않는다.

- 한 상품 안의 컬렉션·컬러·유리 등 서로 다른 선택 축
- 폴더명과 파일 순서가 이루는 이야기 목차
- 전체 상품 글에서 먼저 확인해야 할 필수 선택지
- 일부 주제를 전체 상품처럼 오해하게 만드는 표현 방지 규칙
- 개별 자산이 전체 이야기의 어느 절과 선택지에 속하는지

이 문맥의 `curated_non_authority_context`는 집필 누락을 막는 내부 구성 정보다. 자산 외부 발행 승인, 변동 claim 최신성, 개인정보 판정을 대신하지 않는다.
